...
LOCAL_DB_PATH = os.path.join(CURRENT_DIR, "chroma_db")

if os.path.exists("/tmp") and os.access("/tmp", os.W_OK):
    DB_PATH = "/tmp/chroma_db"
    # Copy committed database from repo to /tmp at startup if /tmp copy doesn't exist
    if not os.path.exists(DB_PATH) and os.path.exists(LOCAL_DB_PATH):
        try:
            import shutil
            shutil.copytree(LOCAL_DB_PATH, DB_PATH)
        except Exception as e:
            print(f"Warning: Failed to copy pre-built database to /tmp: {e}")
else:
